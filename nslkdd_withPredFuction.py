...
class Dataset_nolabel(object):
    def __init__(self, segments, one_hot = False, dtype = dtypes.float32, reshape = True):
        """Construct a Dataset
        one_hot arg is used only if fake_data is True. 'dtype' can be either unit9 or float32
        """
...
def segment_signal_test(data, window_size = 1):
    segments = np.empty((0, window_size, 41))
    num_features = ["duration", "protocol_type","service","flag","src_bytes", "dst_bytes","land","wrong_fragment","urgent","hot","num_failed_logins",
    "logged_in","num_compromised","root_shell","su_attempted","num_root",
    "num_file_creations","num_shells","num_access_files","num_outbound_cmds",
...
def read_data_test(filename):
    ...
    "dst_host_same_srv_rate","dst_host_diff_srv_rate","dst_host_same_srv_port_rate",
    "dst_host_srv_diff_host_rate","dst_host_serror_rate","dst_host_srv_serror_rate",
    "dst_host_rerror_rate","dst_host_srv_rerror_rate", "successfulPrediction"]
    dataset = pd.read_csv(filename, header = None, names = col_names)
    return dataset      
...
def read_data_set_test(dataset3, one_hot = False, dtype = dtypes.float32, reshape = True):
    segments3 = segment_signal_test(dataset3)
    test_x = segments3.reshape(len(segments3), 1, 1 ,41)
    test = Dataset_nolabel(test_x, dtype = dtype, reshape = reshape)
    return base.Datasets(train = test, validation=None, test = test)
...
class DBN(object):
    ...
    def pretraining(self, sess, train_set_x, batch_size = 100, pretraining_epochs = 200, 
        ...
        for i in range(self.n_layers):
            ...
            for epoch in range(pretraining_epochs):
                ...
                #plt.show()
        b = []        
        pr= sess.run(self.pred, feed_dict = {self.x: train_set_x.test.segments})
        b = np.append(b, sess.run(tf.argmax(pr, axis =1)))
        np.savetxt('b.txt', b ,delimiter=',')
        prediction=tf.argmax(pr,1)
        print("predictions", prediction.eval(feed_dict={self.x: train_set_x.test.segments}, session=sess))

        end_time = timeit.default_timer()
        print("time {0} minutes".format((end_time - start_time)/ 60.))

    def predin(self, sess, train_set_x):
        ...
        pr= sess.run(self.pred, feed_dict = {self.x: train_set_x.test.segments})
        b = np.append(b, sess.run(tf.argmax(pr, axis =1)))
        np.savetxt('pred.txt', b ,delimiter=',')
        prediction=tf.argmax(pr,1)
        print("predictions", prediction.eval(feed_dict={self.x: train_set_x.test.segments}, session=sess))
    ...
    def fine_tuning(self, sess, train_set_x, batch_size = 100 , training_epochs = 200 , learning_rate = 0.1, display_step = 1):
        ...
        start_time = timeit.default_timer()
        train_ops = tf.train.GradientDescentOptimizer(learning_rate = learning_rate).minimize(self.finetune_cost, var_list= self.params)
...
def initlabel(dataset):
    ...
    #myserver
    labels[labels == 'nmap'] = 'probe'
    labels[labels == 'namp'] = 'probe'
    labels[labels == 'nmap-dos'] = 'dos'
    labels[labels == 'hping'] = 'dos'
    labels[labels == 'hydra'] = 'u2r'
...
def nominal_test(dataset3):
    protocol3 = dataset3['protocol_type'].copy()
    dataset = dataset3
    protocol_type = dataset['protocol_type'].unique()
    for i in range(len(protocol_type)):
        protocol3[protocol3 == protocol_type[i]] = i
    service3 = dataset3['service'].copy()
    service_type = dataset['service'].unique()
    for i in range(len(service_type)):
        service3[service3 == service_type[i]] = i
    flag3 = dataset3['flag'].copy()
    flag_type = dataset['flag'].unique()
    for i in range(len(flag_type)):
        flag3[flag3 == flag_type[i]] = i
    return protocol3, service3, flag3


if __name__ == "__main__":
    start = timeit.default_timer()
    dir_path = os.path.dirname(os.path.realpath(__file__))
    filename1 = dir_path + "/datasets/KDDTrain+ _MyDataOnly-SS-PC.csv"
    filename2 = dir_path + "/datasets/KDDTest+ _myDataOnly2-SS-PC.csv"
    #filename1 = dir_path + "/datasets/KDDTrain+ _withMyData.csv"
    #filename2 = dir_path + "/datasets/KDDTest+ _withMyData.csv"
    #filename3 = dir_path + "/datasets/normalcapturemerged-PredDataSet.csv"
    filename3 = "c:\\SFTP_Root\\combind.csv"
    dataset1 = read_data(filename1)
    dataset2 = read_data(filename2)
    dataset3 = read_data_test(filename3)
    original_md5 = '5d41402abc4b2a76b9719d911017c592'
    flagfile = "c:\\SFTP_Root\\flag.file"
    ...
    dataset3['protocol_type'], dataset3['service'], dataset3['flag'] = nominal_test(dataset3)
    ...
    print(dataset3['service'].value_counts())


    print(dataset3['protocol_type'].value_counts())
    print(dataset3['flag'].value_counts())
    ...
    print(dataset1['flag'].value_counts())
    ...
    testacc = read_data_set_test(dataset3)
    ...
    with tf.Session() as sess:
        ...
        while var != 1 :  # This constructs an infinite loop
         #   with open(filename3) as file_to_check:
         #       # read contents of the file
         #       data = file_to_check.read()    
         #       # pipe contents of the file through
         # Finally compare original MD5 with freshly calculated
            if os.path.isfile(flagfile):
                print ("NewFileDetected!.")
                print (dataset3.head())
               # var = input("Enter a number  (1 to exit, 2 to continue loop):")
                ...
                dataset3[num_features] = MinMaxScaler().fit_transform(dataset3[num_features].values)
                testacc = read_data_set_test(dataset3)
                dbn.predin(sess, train_set_x = testacc)
                os.remove(flagfile)
            # output = sess.run(pred, {x:acc})
